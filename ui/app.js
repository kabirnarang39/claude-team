  ...
  if (!state.agents.length) {
    ...
    svg.innerHTML = `
      <text x="${w/2}" y="52" text-anchor="middle"
        style="fill:${isRunning ? 'var(--amber)' : 'var(--muted)'};font-family:var(--font);font-size:11px">
        ${esc(msg)}</text>
      ${isRunning ? `<text x="${w/2}" y="72" text-anchor="middle"
        style="fill:var(--muted);font-family:var(--font);font-size:9px">
